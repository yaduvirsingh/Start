...
import java.lang.*;
public class lifegame3 extends JFrame
{
		...
		for(int i=0;i<10;i++)
		{
			...
			for(int j=0;j<30;j++)
			{
				...
				if(i!=0 && sq[i-1][j].preLife)
					count++;
				if(i!=0 && j!=29 && sq[i-1][j+1].preLife==true)
					count++;
				if(j!=0 && sq[i][j-1].preLife==true)
					count++;
				if(j!=29 && sq[i][j+1].preLife==true)
					count++;
				if(i!=9 && j!=0 && sq[i+1][j-1].preLife==true)
					count++;
				if(i!=9 && sq[i+1][j].preLife==true)
					count++;
				if(i!=9&& j!=29 && sq[i+1][j+1].preLife==true)
					count++;
				if(max<count)
					max=count;
				if(count==3)
	...
	public void going()
	{
		...
		if(sq[9][24].x==400)
		{
			...
			for(int i=0;i<10;i++)
			{
				for(int j=0;j<14;j++)
				{
					sq[i][j].newLife=sq[i][j+15].newLife;
					sq[i][j+15].newLife=false;
					sq[i][j].preLife=sq[i][j+15].preLife;
					sq[i][j+15].preLife=false;
				}		
	...
	public lifegame3(String s)
	{
		...
		for(int i=0;i<10;i++)
		{
			for(int j=0;j<30;j++)
			{
				sq[i][j]=new squre(j*40+40,i*40+40,40,40);
			}
		}
	...
	{
		lifegame3 f=new lifegame3("Life Game");
		f.setSize(600,500);
		f.setVisible(true);
